...
public class SpecificCachedUserQueryHandler : IQueryHandler<SpecificCachedUserQuery, UserDTO>
{
    ...
    public async Task<UserDTO> HandleAsync(SpecificCachedUserQuery query)
    {
        try
        {
            var cacheItem = await _distributedCache.GetAsync(query.FullDomainName);
            if (cacheItem != null)
            {
            ...
            var dto = await _queryDispatcher.QueryAsync(new SpecificUserQuery { FullDomainName = query.FullDomainName });
    ...
    public async System.Threading.Tasks.Task StoreInCache(UserDTO entity)
    {
